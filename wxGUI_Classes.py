...
class vFrame(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        #seek slider
        seek_label = wx.StaticText(self.Panel,-1,"Seek %",size=(100,-1), style=wx.ALIGN_CENTER)
        self.seek_sld = wx.Slider(self.Panel,-1,value=0,minValue=0,maxValue=100,style=wx.SL_HORIZONTAL | wx.SL_AUTOTICKS | wx.SL_LABELS)
        self.seek_sld.SetTickFreq(10,1)
        self.seek_sld.Bind(wx.EVT_SCROLL_CHANGED, self.onSkSlider)
        self.PlayBackSizer.AddSpacer(10)
        self.PlayBackSizer.Add(seek_label)
        self.PlayBackSizer.Add(self.seek_sld)
        ...
        self.val_CB = wx.CheckBox(self.Panel,-1,"Write Tracker Video")
        self.Bind(wx.EVT_CHECKBOX,self.trVid_CB,self.val_CB)

        self.CBSizer.AddSpacer(45)
        self.CBSizer.Add(self.val_CB)
...
#        cFrame[:,:,0] = frame1GS#background updated for each track on batch tracking
    ...
    def stopTracking(self):
        ...
        if GlProp.vidN+1 < len(GlProp.vidfile):
            ...
            try:
                ...
                pass
            #wx.PostEvent(self.GetEventHandler(),wx.EVT_CHECKBOX)
    ...
    def setupBatch(self,e):
        ...
        dlg = wx.DirDialog(self,"Select directory containing sequential video files:",
                           defaultPath=self.dirname, style=wx.DD_DEFAULT_STYLE)
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def trVid_CB(self,e):
        ...
        if e.IsChecked():
            try:
                GlProp.vidWriter = InitializeVideoOutput(self.getTrckVid_Name(),(cFrame.shape[1],cFrame.shape[0]))
                GlProp.fps = 60.
                print "Avoid checking display windows and writing video simultaneously..."
            except:
                print "No video capture has been defined"
        else:
            GlProp.vidWriter.release()
            GlProp.vidWriter = None
            GlProp.fps=1000.
...
class GlobalProperty:
    ...
    def __init__(self, TrackingThreshold, SizeThreshold, alpha, fps, mDist, bgRatio, cvCheckBoxDict):
        ...
        self.vidWriter = None #contains the videowriter object for creating validation video
        self.frametimer = []
...
def SetImg_Globals((width,height)):
    ...
    global cFrame
    cFrame = np.empty((width,height,1),dtype="uint8")
