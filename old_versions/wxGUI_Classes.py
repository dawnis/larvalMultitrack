...
#        self.CBSizer.Add(inSelector,1,wx.EXPAND)
    ...
    def onNextFrame(self,e):
        ...
        ret,cFrame[:,:,1]=GetGSFrame(GlProp.vidstream)
        ...
        if ret & setpos & (GlProp.fIdx+GlProp.fDir > -1):
            cFrame[:,:,2]=CalculateBGDiff(cFrame[:,:,0],cFrame[:,:,1],GlProp.trckThrsh,GlProp.TrckParameter)
            #frameImg.append(dFrame)#2
            CurContours = updateTracking(GlProp.fIdx,cFrame[:,:,2],minSize=GlProp.szThrsh,minDist=GlProp.mDist,currObjs=tObjList,trackData=GlProp.trackData)
            if GlProp.fIdx % GlProp.bgRatio == 0: #only update background if the frame # is a multiple of the bgRatio
                cFrame[:,:,0] = BGUpdateFnc(cFrame[:,:,0],cFrame[:,:,1],tObjList,CurContours,GlProp.alpha)
            if GlProp.cvCB["TrackWindow"]:
                #frameImg.append(draw_tObjs(frame,CurContours,tObjList,GlProp.mDist))#3
                cFrame[:,:,GlProp.cvFrame["TrackWindow"]] = draw_tObjs(cFrame[:,:,1],CurContours,tObjList,GlProp.mDist)#3
            for cvWindow in GlProp.cvCB:
