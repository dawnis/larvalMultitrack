...
class vFrame(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        GlProp = GlobalProperty(TrackingThreshold=15, SizeThreshold=3., alpha=20, fps=1000., neighborDist = 10, cvCheckBoxDict=cvCheckBoxDict)
        #NOTES
...
#        self.CBSizer.Add(inSelector,1,wx.EXPAND)
        ...
        size_slider,sizeSlider_obj = makeGl_Slider(self.Panel,GlProp.szThrsh,(0,50),"szThrsh","Size Threshold")
    ...
    def onNextFrame(self,e):
        ...
        if ret & setpos & (GlProp.fIdx+GlProp.fDir > -1):
            cFrame[:,:,0] = BGUpdateFnc(cFrame[:,:,0],frame,GlProp.alpha)
            dFrame=CalculateBGDiff(cFrame[:,:,0],frame,GlProp.trckThrsh)
            frameImg.append(dFrame)#2
            CurContours = updateTracking(GlProp.fIdx,dFrame,minSize=GlProp.szThrsh,minDist=50.,currObjs=tObjList,trackData=GlProp.trackData)
            if GlProp.cvCB["TrackWindow"]:
                frameImg.append(draw_tObjs(frame,CurContours,tObjList,GlProp.neighborDist))#3
            for cvWindow in GlProp.cvCB:
    ...
    def onTrack(self,e):
        ...
        logfile = self.dirname +"\\"+re.split('\.', self.filename)[0]+".pickle"
        for button in self.playctrlbuttons:
            button.Disable()
        GlProp.output = open(logfile,'wb')
        GlProp.trackingON = True
        GlProp.frametimer.Start()
    ...
    def OnOpen(self,e):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            print "Video File: "+self.filename
    ...
    def setupBatch(self,e):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            GlProp.batchdir = dlg.GetPath()
            print "Batch Tracking Set for Directory: "+GlProp.batchdir
        dlg.Destroy()
