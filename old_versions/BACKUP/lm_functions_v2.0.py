...
def updateTracking(binIMG,minSize,minDist,currObjs):
    """updates the tracking information based on the binary threshold image and current list of tracked objects. Returns contours and updated tObj list"""
...
def DetectObjects(binIMG,minSize):
    ...
    contourlist = np.empty((100,3),dtype=int) 
    (contours,hierarchy)=cv2.findContours(np.array(binIMG),cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
    dCount = 0
