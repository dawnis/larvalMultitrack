...
class vFrame(wx.Frame):
    ...
    def onNextFrame(self,e):
        ...
        if ret & setpos:
            ...
            CurContours = updateTracking(dFrame,minSize=GlProp.szThrsh,minDist=50.,currObjs=tObjList)
            #if GlProp.cvCB["TrackWindow"]:
            tFrame = draw_tObjs(frame,CurContours,tObjList,GlProp.neighborDist)
            #cv2.imshow("trackFrame",trackFrame)
            for cvWindow in GlProp.cvCB:
                if GlProp.cvCB[cvWindow]:
                    cv2.imshow(cvWindow,tFrame)
