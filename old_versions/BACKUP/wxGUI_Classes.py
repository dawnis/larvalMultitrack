...
class vFrame(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.cFrame = set_cFrame((1000,1000)) #stores all frame data
        global GlProp
        cvCheckBoxDict={"BackgroundWindow":0,"LiveStream":1,"DifferenceWindow":2,"TrackWindow":[3,4,5]} #defines the index of each window in cFrame (local frames in on next)
        ...
        #self.Bind(wx.EVT_MENU,lambda evt, cFrame=self.cFrame : self.OnOpen(evt,cFrame), f_open)
    ...
    def onNextFrame(self,e):
        frPrStart=time.time()
        Frame = np.empty((self.cFrame.shape[0],self.cFrame.shape[1],6),dtype="uint8")
        #print [tObj.kID for tObj in tObjList.itervalues()]
        #ret,frame=GlProp.vidstream.read()
        ret, Frame[:,:,GlProp.cvFrame["LiveStream"]] = GetGSFrame(GlProp.vidstream)
        #print GlProp.fIdx
        ...
        if ret & setpos:
#            self.cFrame[:,:,GlProp.cvFrame["LiveStream"]] = frame
#            self.cFrame[:,:,GlProp.cvFrame["BackgroundWindow"]] = BGUpdateFnc(self.cFrame[:,:,0],self.cFrame[:,:,1],GlProp.alpha)
#            self.cFrame[:,:,GlProp.cvFrame["DifferenceWindow"]]=CalculateBGDiff(self.cFrame[:,:,0],self.cFrame[:,:,1],GlProp.trckThrsh)
#            CurContours = updateTracking(self.cFrame[:,:,2],minSize=GlProp.szThrsh,minDist=GlProp.neighborDist,currObjs=tObjList)
            Frame[:,:,GlProp.cvFrame["BackgroundWindow"]] = BGUpdateFnc(self.cFrame,Frame[:,:,GlProp.cvFrame["LiveStream"]],GlProp.alpha)
            Frame[:,:,GlProp.cvFrame["DifferenceWindow"]] = CalculateBGDiff(self.cFrame,Frame[:,:,GlProp.cvFrame["LiveStream"]],GlProp.trckThrsh)
            CurContours = updateTracking(Frame[:,:,GlProp.cvFrame["DifferenceWindow"]],minSize=GlProp.szThrsh,minDist=GlProp.neighborDist,currObjs=tObjList)
            if GlProp.cvCB["TrackWindow"]:
                #self.cFrame[:,:,GlProp.cvFrame["TrackWindow"]] = draw_tObjs(self.cFrame[:,:,1],CurContours,tObjList,GlProp.neighborDist)
                Frame[:,:,GlProp.cvFrame["TrackWindow"]] = draw_tObjs(Frame[:,:,GlProp.cvFrame["LiveStream"]],CurContours,tObjList,GlProp.neighborDist)
            for cvWindow in GlProp.cvCB:
                if GlProp.cvCB[cvWindow]:
                    cv2.imshow(cvWindow,Frame[:,:,GlProp.cvFrame[cvWindow]])
            self.cFrame = Frame[:,:,GlProp.cvFrame["BackgroundWindow"]] 
        else:
    ...
    def update_processRate(self,e):
        ...
        GlProp.processRate = np.zeros(2,dtype=float)
    ...
    def OnOpen(self,e):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.cFrame = frame1GS
            #self.cFrame[:,:,1] = frame1GS.copy()
            #sbmp=wx.StaticBitmap(self,-1,bitmap=self.bmp)
            self.Refresh()
        dlg.Destroy()
...
class cvWindowCheckBox(wx.CheckBox):
    ...
    def EvtCheckBox(self,e,chLabel):
        ...
        if e.IsChecked():
            ...
            #cv2.imshow(chLabel,self.parent.Parent.cFrame[:,:,GlProp.cvFrame[chLabel]])
        else:
...
def set_cFrame((width,height)):
    return np.empty((width,height),dtype="uint8")

#def SetImg_Globals((width,height)):
#    """Defines globals for containing the frames"""
#    global bgFrame
#    bgFrame = np.empty((width,height),dtype="uint8")
